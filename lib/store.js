...
export class ImprovementStore {
    ...
    findPromotion(signature) {
        return this.state.promotions.find((promotion) => promotion.signature === signature);
    }
    promote(entry, maxRules) {
        const existing = this.findPromotion(entry.signature);
        if (existing)
            return existing;
        const promotion = { ...entry, promotedAt: Date.now() };
        ...
        while (this.state.promotions.length > Math.max(1, maxRules)) {
            const index = this.state.promotions.findIndex((candidate) => !candidate.lessonId);
            if (index < 0)
                break;
