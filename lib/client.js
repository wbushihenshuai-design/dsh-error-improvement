window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		async function readConfiguredModelCatalog(ctx) {
			try {
				const binder = ctx?.settingsScope;
				if (typeof binder?.bind !== "function") return [];
				const scope = binder.bind({ namespace: "llm-pi-ai" });
				await scope.load?.();
		...
		async function readModelCatalog(ctx) {
			try {
				const models = await readConfiguredModelCatalog(ctx);
				const connection = ctx?.get?.("connection") ?? ctx?.connection;
				if (!connection?.api) return models;
				if (typeof connection.api.llm?.discoverModels === "function") {
					...
					const discovered = normalizeModelList(value?.models ?? value);
					const known = new Set(models.map((model) => model.id));
					for (const model of discovered) {
						if (!known.has(model.id)) models.push(model);
					}
				}
				if (
					models.length === 0 &&
