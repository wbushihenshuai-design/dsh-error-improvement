...
export const builtinLessons = Object.freeze([
    {
        id: "builtin-no-repeated-tool-calls",
        title: "Avoid repeated identical tool calls",
        mistake: "Fired 3+ consecutive identical tool calls (same grep, Select-String, or search), triggering the framework's anti-loop abort mechanism.",
        prevention: "Limit consecutive identical tool calls to 2; on the 3rd attempt change approach (read the file directly, combine into one command, or use a different method).",
...
export const ErrorImprovementSettingsSchema = z.object({
    ...
    mode: z.union(["assist", "strict"]).default("assist"),
    ...
    enforcement: z
        ...
        defaultMode: z.union(["warn", "deny"]).default("warn"),
...
function terms(value) {
    ...
    for (const token of normalized.match(/[\p{L}\p{N}_-]+/gu) ?? []) {
        if (token.length >= 2)
            result.add(token);
        const cjkChunks = token.match(/[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]+/gu);
...
export function relevanceScore(lesson, query) {
    ...
    if (explicit) {
        ...
        for (const term of terms(explicit)) {
            if (queryTerms.has(term))
                explicitScore += term.length >= 4 ? 4 : 3;
        }
...
export function safeField(value, maxLength = 2000) {
...
function completeLessons(settings) {
...
export function selectLessons(settings, query) {
    const maximum = finiteInteger(settings.maxLessons, defaultSettings.maxLessons, 1, 50);
    const lessons = completeLessons(settings);
    if (settings.mode === "strict")
        return lessons.slice(0, maximum);
    return (lessons
        .map((lesson, index) => ({
        lesson,
        index,
...
export function renderLessons(settings, query) {
    if (settings.enabled === false)
        return undefined;
    const maximum = finiteInteger(settings.maxChars, defaultSettings.maxChars, 500, 50_000);
...
export function blocksToText(blocks) {
    ...
    for (const block of blocks) {
        if (block.type === "text")
            ...
            output.push(imageText(block));
        else if (block.type === "tool-result")
            output.push(blocksToText(block.content));
    }
    return output.filter(Boolean).join("\n").trim();
}
export function directUserQuery(messages) {
    return messages
...
export function isLessonMessage(message) {
    return (message?.source?.kind === "plugin" && message.source.plugin === PLUGIN_NAME);
}
export function lessonMessage(settings, messages) {
