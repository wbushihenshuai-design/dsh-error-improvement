...
1. Host 监听 `tools/post-execute`，对**相同工具 + 相同错误签名**的失败计数（错误签名 = 归一化错误头部；计数持久化在 `$DSH_HOME/error-improvement/state.json`，重启不丢）。
...
4. 所有钩子**失败开放**（插件异常绝不阻断工具流水线），也绝不会把下游已有的 deny/ask 决策放宽成 allow。规则上限 `maxRules`（默认 20），超出时只驱逐最旧的自动规则，关联教训的规则不驱逐。

### 成功配方沉淀（0.2.0）

- 解决非平凡问题后，agent 可调用 **`improve_record_recipe`** 工具把**已验证可行**的方案写入配方库（标题 / 问题 / 方案 / 范围 / 关键词），持久化到同一 state.json。
- 配方与教训共用同一套相关性引擎（含中文二元组），以 `<success_recipes>` 块和教训一起注入，遇到同类问题时**直接复用已验证方案，不再重新造轮子**。
- 记录时 `asSkill: true` 会把配方**毕业**为独立技能文件 `$DSH_HOME/skills/<slug>/SKILL.md`，从"经验"升级为"能力"。
- 也可在设置的 `recipes[]` 节中手工维护配方（同样需 `confirmed` 才会注入）。
...
| `maxChars` | `6000` | 最大完整教训块，限制在 500–50000 字符（教训与配方共享预算）。 |
| `maxRecipes` | `3` | 每轮最多注入的配方数，限制在 1–20。 |
| `lessons[]` | `[]` | 用户管理的教训记录。 |
| `recipes[]` | `[]` | 用户管理的成功配方记录。 |
| `enforcement.enabled` | `true` | 启用重复错误计数与前置拦截。 |
| `enforcement.threshold` | `3` | 同一错误出现多少次后升级为规则，限制在 2–10。 |
| `enforcement.defaultMode` | `warn` | 新规则的默认模式：`warn` 单次提醒 / `deny` 硬拦截。 |
| `enforcement.warnCooldownMs` | `3600000` | 同一规则两次 warn 拦截的最小间隔（毫秒）。 |
| `enforcement.maxRules` | `20` | 规则数量上限；超出时驱逐最旧的自动规则。 |
