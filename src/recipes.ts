...
function recipeScore(recipe: SuccessRecipe, query: string): number {
...
export function completeRecipes(
	recipes: readonly SuccessRecipe[],
): SuccessRecipe[] {
	const seenIds = new Set<string>();
	const result: SuccessRecipe[] = [];
...
export function recipeSlug(title: string, fallbackId: string): string {
	const slug = title
		.normalize("NFKC")
		.toLocaleLowerCase()
		.replace(/[^a-z0-9]+/gu, "-")
...
export function graduateRecipe(recipe: SuccessRecipe): string {
...
export function renderAll(
	...
	const recipesText =
		recipes.length > 0 && remaining > 200
			? renderRecipes(
					recipes,
					query,
					remaining,
					clampInt(settings.maxRecipes, defaultSettings.maxRecipes, 1, 20),
...
export function improvementMessage(
	settings: ErrorImprovementSettings,
...
export function recordRecipe(
	...
	args: RecordRecipeArgs,
): { message: string; id: string; skillPath?: string } {
