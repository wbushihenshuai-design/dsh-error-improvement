...
export class ConfigurableCompactionEngine extends BasicCompactionEngine {
	...
	constructor(ctx: Context, getSettings: () => ErrorImprovementSettings) {
		...
		// Shallow-copy the frozen base config into a mutable object so the Proxy
		// target's properties are configurable; otherwise the JS Proxy invariant
		// check rejects a get-trap that returns a different frozen array reference
		// for `modelPolicies` when an async operation swaps the active config.
		this.#defaultConfig = { ...this.config } as ResolvedConfig;
